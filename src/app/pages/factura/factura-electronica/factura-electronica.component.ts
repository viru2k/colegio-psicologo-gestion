...
import { PopupFacturaRenglonComponent } from "./popups/popup-factura-renglon/popup-factura-renglon.component";
 ...
 */
  ...
  generarPDF() {
    ...
    for (i = 0; i < this.elementosPDF.length; i++) {
      this.elementosPDF[i]["alicuota"] = Number(
        this.elementosPDF[i]["alicuota"]
      );
      this.elementosPDF[i]["iva"] = Number(this.elementosPDF[i]["iva"]);
      this.elementosPDF[i]["precio_unitario"] = Number(
        this.elementosPDF[i]["precio_unitario"]
      );
      this.elementosPDF[i]["total_renglon"] = Number(
        this.elementosPDF[i]["total_renglon"]
      );
      this.elementosPDF[i]["total_sin_iva"] = Number(
    ...
    //doc.text('VILLAFAÑE GASTON ALEJANDRO', 15, 35);
    doc.setFontStyle("normal");
    doc.setFontSize(9);
