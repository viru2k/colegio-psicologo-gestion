...
export class LiquidarAccionesComponent implements OnInit {
  ...
  sumarTotal(_elementos) {
    ...
    for (i = 0; i < _elementos.length; i++) {
      ...
      this.TOTAL_DEDUCCIONES =
        this.TOTAL_DEDUCCIONES +
        Number(_elementos[i].os_ing_brutos) +
        Number(_elementos[i].os_lote_hogar) +
        Number(_elementos[i].os_gasto_admin) +
        Number(_elementos[i].os_imp_cheque);
      this.TOTAL_CONCEPTOS =
        this.TOTAL_CONCEPTOS +
        Number(_elementos[i].os_desc_matricula) +
        Number(_elementos[i].os_desc_fondo_sol) +
        Number(_elementos[i].os_otros_ing_eg);
      ...
      this.liquidacionService
        ...
        .subscribe(
          (resp) => {
            ...
            this.excelService.exportAsExcelFile(
              exportar,
              "listado_al_dia_al_" + this._fecha
            );
            this.loading = false;
          },
          (error) => {
  ...
  getExpedienteByIdLiquidacion() {
    ...
    try {
      this.liquidacionService
        .getLiquidacionDetalleByidLiquidacion(String(this.id_liquidacion))
        .subscribe(
          (resp) => {
            this.elementos = resp;
            this.sumarTotal(this.elementos);
  ...
  getProximoNumeroLiquidacion() {
    ...
    try {
      this.liquidacionService
        .getUltimoNroIngBrutos(String(this.id_liquidacion))
        .subscribe(
          (resp) => {
            console.log(resp);
  ...
  getProximoNumeroRecibo() {
    ...
    try {
      this.liquidacionService
        .getUltimoNroRecibo(String(this.id_liquidacion))
        .subscribe(
          (resp) => {
            console.log(resp);
  ...
  GenerarRecibo() {
    swal({
      ...
      if (result.value) {
        ...
        try {
          this.liquidacionService
            .putActualizarNroRecibo(
              this.selecteditems,
              this.proximo_numero_recibo
            )
            .subscribe(
              (resp) => {
                console.log(resp);
  ...
  GenerarIngresosBrutos() {
    swal({
      ...
      if (result.value) {
        ...
        try {
          this.liquidacionService
            .putActualizarNroIngBrutos(this.selecteditems, this.proximo_numero)
            .subscribe(
              (resp) => {
                console.log(resp);
                this.loading = false;
  ...
  liquidacionByMatricula(psicologo: any) {
    ...
    try {
      this.liquidacionService
        .getActuacionProfesionalByMatricula(psicologo.mat_matricula_psicologo)
        .subscribe(
          (resp) => {
            console.log(resp);
            this.excelService.exportAsExcelFile(
  ...
  liquidar() {
    try {
      this.liquidacionService
        .liquidar(String(this.id_liquidacion), this.descuenta_matricula)
        .subscribe(
          (resp) => {
            console.log(resp);
            this.loading = false;
  ...
  imprimirActuacionProfesional() {
    ...
    doc.setFontSize(10);
    doc.text(
      "Detalle de actuación profesional",
      pageWidth / 2,
      10,
      null,
      null,
    ...
    doc.text(
      this.selecteditems[0].mat_apellidoynombre,
      pageWidth - 60,
      16,
      null,
      null
  ...
  excelPsicologo() {
    ...
    this.selecteditems.forEach((element) => {
      ...
      seleccionados["FECHA_PAGO"] = formatDate(
        element.os_fecha,
        "dd/MM/yyyy",
        "es-Ar"
      );
    ...
    this.excelService.exportAsExcelFile(
      exportar,
      "listado_pago_banco " +
        this.selecteditems[0].mat_banco_nombre +
        fecha_impresion
    );
  }
  ...
  excelProveedor() {
    ...
    this.selecteditems.forEach((element) => {
      ...
      seleccionados["FECHA_EMISION"] = formatDate(
        element.os_fecha,
        "dd/MM/yyyy",
        "es-Ar"
      );
    ...
    this.excelService.exportAsExcelFile(
      exportar,
      "listado_pago_banco_proveedores " +
        this.selecteditems[0].mat_banco_nombre +
        fecha_impresion
    );
  }
  ...
  generarPdfRentas(elem: any) {
    ...
    const _fechaEmision = formatDate(
      this.selecteditem["os_fecha"],
      "dd/MM/yyyy",
      "en"
    );
    ...
    doc.text(
      "CERTIFICADO DE RETENCION Nº " + this.selecteditem["os_num_ing_bruto"],
      pageWidth - 80,
      15
    );
    ...
    doc.text(
      "Nº ing. brutos: " + this.selecteditem["mat_ning_bto"],
      pageWidth - 80,
      60
    );
    doc.text(
      "Domicilio: " + this.selecteditem["mat_domicilio_particular"],
      pageWidth - 80,
      65
    );
    doc.line(10, 70, pageWidth - 10, 70);
    const imp_retenido =
      Number(this.selecteditem["os_ing_brutos"]) +
      Number(this.selecteditem["os_lote_hogar"]);
    doc.text(
      "Monto imponible: " +
        this.cp.transform(
          this.selecteditem["os_liq_bruto"],
          "",
          "symbol-narrow",
          "1.2-2"
        ),
    ...
    doc.text(
      "Ing. brutos: " +
        this.cp.transform(
          this.selecteditem["os_ing_brutos"],
          "",
          "symbol-narrow",
          "1.2-2"
        ),
    ...
    doc.text(
      "Lote hogar: " +
        this.cp.transform(
          this.selecteditem["os_lote_hogar"],
          "",
          "symbol-narrow",
          "1.2-2"
        ),
    ...
    doc.text(
      "Importe retenido: " +
        this.cp.transform(imp_retenido, "", "symbol-narrow", "1.2-2"),
      130,
      75
    );
