...
export class PopupLiquidacionExpedienteEditarComponent implements OnInit {
  ...
  constructor(public config: DynamicDialogConfig, private liquidacionService: LiquidacionService,
...
guardarDatos() {
  ...
  try {
  this.liquidacionService.putLiquidacionDetalle( this.updateDataForm.value, this.updateDataForm.value.id_liquidacion_detalle)
  .subscribe(resp => {
  this.loading = false;
  console.log(resp);
  this.ref.close(resp);
  ...
  sumarDeuda() {
    let total =  Number(this.updateDataForm.value.os_liq_bruto )
