...
export class PopupLiquidacionGeneradaDetalleComponent implements OnInit {
  ...
  constructor(
    ...
    this.columnsOrden = [
      ...
      { title: "valor", dataKey: "os_precio_sesion" },
  ...
  ngOnInit() {
    ...
    this.getDeudaByMatriculaAndEstado(
      this.config.data.mat_matricula,
      "P",
      this.config.data.id_liquidacion_detalle
    );
    this.getOrdenByMatriculaAndLiquidacion(
      this.config.data.mat_matricula,
      this.config.data.id_liquidacion_generada
    );
  }
              ...
              for (i = 0; i < resp.length; i++) {
                ...
                if (
                  ...
                  resp[i]["mat_monto_final"] =
                    Number(resp[i]["mat_monto"]) *
                    Number(resp[i]["mat_interes"]);
                  this.total =
                    this.total +
                    Number(resp[i]["mat_monto"]) *
                      Number(resp[i]["mat_interes"]);
  ...
  imprimirResumen() {
    ...
    for (let i = 0; i < this.elementosOS.length; i++) {
      _obra_social =
        _obra_social +
        " | " +
        this.elementosOS[i].os_nombre +
        " - " +
        formatDate(this.elementosOS[i].os_fecha_desde, "MM/yyyy", "en");
    }
    ...
    doc.setFontSize(8);
    doc.text(
      "Liquidación Nro : " + this.config.data.id_liquidacion,
      pageWidth - 60,
      10,
      null,
      null
    ...
    doc.text(
      "Fecha de liq. : " +
        formatDate(this.config.data.os_fecha, "dd/MM/yyyy", "en"),
      pageWidth - 60,
      13,
      null,
      null
    ...
    doc.text(
      "Comprobante : " + this.config.data.num_comprobante,
      pageWidth - 60,
      16,
      null,
      null
    ...
    doc.text(
      "Matricula : " + this.config.data.mat_matricula,
      pageWidth - 60,
      19,
      null,
      null
    ...
    doc.text(
      this.config.data.mat_apellidoynombre,
      pageWidth - 60,
      22,
      null,
      null
    ...
    doc.text(
      "Ing. brutos: " +
        this.cp.transform(
          this.config.data.os_ing_brutos,
          "",
          "symbol-narrow",
          "1.2-2"
        ),
    ...
    doc.text(
      "Lote hogar: " +
        this.cp.transform(
          this.config.data.os_lote_hogar,
          "",
          "symbol-narrow",
          "1.2-2"
        ),
    ...
    doc.text(
      "Gastos adm.: " +
        this.cp.transform(
          this.config.data.os_gasto_admin,
          "",
          "symbol-narrow",
          "1.2-2"
        ),
    ...
    doc.text(
      "Imp. cheque: " +
        this.cp.transform(
          this.config.data.os_imp_cheque,
          "",
          "symbol-narrow",
          "1.2-2"
        ),
    ...
    doc.text(
      "Matricula: " +
        this.cp.transform(
          this.config.data.os_desc_matricula,
          "",
          "symbol-narrow",
          "1.2-2"
        ),
    ...
    doc.text(
      "Fondo solidario: " +
        this.cp.transform(
          this.config.data.os_desc_fondo_sol,
          "",
          "symbol-narrow",
          "1.2-2"
        ),
    ...
    doc.text(
      "Otros descuentos: " +
        this.cp.transform(this.config.data.os_descuentos, "", "", "1.2-2"),
      105,
      currentY + 24,
      null,
      null
  ...
  sumarValoresSeleccionados(vals: any) {
    ...
    for (i = 0; i < vals.length; i++) {
      this.total_seleccionado =
        this.total_seleccionado + Number(vals[i]["mat_monto_final"]);
    }
  }
